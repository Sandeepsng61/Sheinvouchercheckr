...
DEFAULT_PRODUCT_ID = "443316088007"
DEFAULT_QUANTITY = 1
...
def fetch_variants(session: requests.Session, product_id: str) -> List[Dict[str, Any]]:
    ...
    # Debug logging
    logging.info(f"Fetched {len(variants)} variants for product {product_id}")
    for i, v in enumerate(variants):
        logging.info(f"Variant {i}: size={v.get('size')}, stock={v.get('stock')}, code={v.get('code')}")
...
def choose_variant(variants: List[Dict[str, Any]], allowed_sizes: List[str]) -> Optional[Dict[str, Any]]:
    ...
    if not variants:
        logging.warning("No variants provided to choose_variant")
        return None
    
    allowed_upper = {str(s).strip().upper() for s in allowed_sizes}
    logging.info(f"Looking for sizes: {allowed_upper}")
    
    # First priority: exact match with allowed sizes
    for v in variants:
        size_value = str(v.get("size", "")).strip().upper() if v.get("size") else ""
        stock = v.get("stock", 0)
        logging.info(f"Checking variant: size='{size_value}', stock={stock}")
        
        if size_value in allowed_upper and stock > 0:
            logging.info(f"✅ Found preferred size: {size_value}")
            return v
    
    logging.info("No preferred sizes available, looking for any in-stock variant...")
    ...
    for v in variants:
        ...
        if stock > 0:
            size_value = str(v.get("size", "")).strip().upper() if v.get("size") else ""
            logging.info(f"✅ Found available size: {size_value}")
            return v
    
    logging.warning("❌ No variants with stock > 0 found")
    return None
...
async def show_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"📦 Quantity: <code>{config['quantity']}</code>\n"
        f"👕 Sizes: <code>{', '.join(config['sizes'])}</code>\n\n"
        f"<i>Note: If preferred sizes are unavailable, any available size will be used for voucher validation.</i>\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "*IMPORTANT:*\n"
        "• If your preferred sizes are unavailable, the bot will use any available size.\n"
        "• This ensures voucher validation works even with different sizes.\n\n"
        "*Default Product:* Jeans (ID: 443323084003)\n"
        "Change with /set_product <new_id>",
        parse_mode="Markdown",
        reply_markup=create_main_keyboard(user_id)
    )
    ...
    if text == "⚙️ Settings":
        ...
        if not config.get('cookie'):
            ...
            await update.message.reply_text(
                ...
                "Example:\n"
                "```\nSVW123456\nSVE789012\nSVF345678\n```\n\n"
                "*Note:* Uses available size if preferred ones are out of stock.\n\n"
        ...
        await update.message.reply_text(
            ...
            "*Size Flexibility:*\n"
            "If your preferred sizes are out of stock, the bot will automatically use any available size to ensure voucher validation works.\n\n"
            "*Troubleshooting:*\n"
            "If 'No sizes in stock' error:\n"
            "1. Try /set_product <new_product_id>\n"
            "2. Try different sizes: /set_sizes S M L XL\n\n"
            "Use 🔙 Back to Menu.",
            parse_mode="Markdown",
            reply_markup=create_main_keyboard(user_id)
        )
